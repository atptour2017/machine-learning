...
if __name__ == '__main__':
	rawlist,raw_dataset=getDataset('new_wine.data')
	training_list,test_list=random_div(rawlist,50)
	training_dataset=np.array(training_list)
	test_dataset=np.array(test_list)
	training_X=training_dataset[:,0:13].astype(np.float64)
	training_y=training_dataset[:,13]
	test_X=test_dataset[:,0:13].astype(np.float64)
