...
def calMatConP(mat,attrlist,label,disperseTag): 	#计算一个矩阵里的每列的概率分布，attrlist是属性列表
	m=mat.shape[1]		#属性个数
	p=list()
	for i in range(m):	#按属性，逐列处理 
		if disperseTag[i]=='disperse':		#离散数据
			...
			for value in tmplist:
				...
				if label=='':
					print num
				posibility=float(num)/float(mat.shape[0])
				posi.append(posibility)
			#print posi
			p.append(posi)
		else:		#连续数据
			average=mat.T[i].mean()		#均值:高斯分布数学期望
			variance=var(mat.T[i])
...
class Bayes(object):		#通用朴素贝叶斯分类类
	...
	def trainNativeBayes(self):		#根据训练数据计算条件概率
		...
		for label in self.labels:
			...
			p=list()
			p=calMatConP(labeldatamat,self.attrlist,label,self.disperseTag)		#求得这个标签下所有属性所有值的概率
			pcon.append(p)
		self.conp=pcon		#条件概率
