...
def getDataset():
	...
	"""
	tmp = np.loadtxt("pima-indians-diabetes.data", dtype=np.str, delimiter=",")
	data = tmp[:,0:7].astype(np.float64)
	label = tmp[:,8].astype(np.int8)
	"""
...
if __name__ == '__main__':
	rawlist,raw_dataset=getDataset()
	#training_list,test_list=end_div(rawlist,20)
	training_list,test_list=random_div(rawlist,10)
