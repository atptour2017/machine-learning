...
def random_div(raw_list,percent):
	training_list=copy.copy(raw_list)
...
def begin_div(raw_list,percent):
	training_list=copy.copy(raw_list)
        length=len(training_list)
        len_test=int(length*percent/100)
...
def end_div(raw_list,percent):
	training_list=copy.copy(raw_list)
        length=len(training_list)
        len_test=int(length*percent/100)
        test_list=training_list[len_test:length]
        training_list=training_list[0:len_test]
        return training_list,test_list
